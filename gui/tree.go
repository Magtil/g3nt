...
func (t *Tree) InsertAt(pos int, child IPanel, using ...interface{}) {
	ti := newTreeItem(child, using)
	t.List.InsertAt(pos, ti)
}

// Add child panel to the end tree.
func (t *Tree) Add(ichild IPanel, using ...interface{}) {
	ti := newTreeItem(ichild, using)
	t.List.Add(ti)
}
...
func (t *Tree) InsertNodeAt(pos int, text string, using ...interface{}) *TreeNode {
...
func (t *Tree) AddNode(text string, using ...interface{}) *TreeNode {
	n := newTreeNode(text, t, nil)
	n.Using = using
	n.update()
	n.recalc()
...
func newTreeItem(ichild IPanel, using []interface{}) IPanel {
	ti := new(TreeItem)
	ti.IPanel = ichild
	if len(using) > 0 {
		ti.Using = using
...
func (n *TreeNode) GetExpanded() bool {
	return n.expanded
}
...
func (n *TreeNode) InsertAt(pos int, child IPanel, using ...interface{}) {
	//fmt.Printf("InsertAt:%v\n", using)
	...
	if len(using) > 0 {
		if len(using) == 1 && reflect.TypeOf(using[0]).String() == "[]interface {}" {
			ti.Using = using[0]
		} else {
			ti.Using = using
		}
...
func (n *TreeNode) Add(child IPanel, using ...interface{}) {
	//fmt.Printf("Add:%v\n", using)
	n.InsertAt(n.Len(), child, using)
}

// InsertNodeAt inserts a new node at the specified position in this node
// If the position is invalid, the function panics
func (n *TreeNode) InsertNodeAt(pos int, text string, using ...interface{}) *TreeNode {
	//fmt.Printf("InsertNodeAt:%v\n", using)
	...
	if len(using) > 0 {
		if len(using) == 1 && reflect.TypeOf(using[0]).String() == "[]interface {}" {
			childNode.Using = using[0]
		} else {
			childNode.Using = using
		}
	}
...
func (n *TreeNode) AddNode(text string, using ...interface{}) *TreeNode {
	//fmt.Printf("AddNode:%v\n", using)
	return n.InsertNodeAt(n.Len(), text, using)
}
